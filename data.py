...
class Cifar10Dataset(Dataset):  # BeanDataset
    def __init__(self, image_processor, tp_size=1, split="train"):
        super().__init__()
        self.image_processor = image_processor
        self.ds = load_dataset("cifar10")[split]  # beans
    ...
    def process_example(self, example):
        input = self.image_processor(example["img"], return_tensors="pt")  # image
        input["labels"] = example["label"]  # labels
        return input


def cifar_collator(batch):  # bean_collator
    return {
